...
export function ActivePackagesSessionsCard({
  ...
  const safeTotalActive =
    typeof totalActive === "number" && Number.isFinite(totalActive)
      ? totalActive
      : 0;
  const safeCompletedSessions =
    typeof completedSessions === "number" && Number.isFinite(completedSessions)
      ? completedSessions
      : 0;
  const safeTotalSessions =
    typeof totalSessions === "number" && Number.isFinite(totalSessions)
      ? totalSessions
      : 0;

  const progress =
    safeTotalSessions > 0 ? safeCompletedSessions / safeTotalSessions : 0;
  const clamped =
  ...
  const rawPercent =
    safeTotalSessions > 0 ? (safeCompletedSessions / safeTotalSessions) * 100 : 0;
          ...
          <View className="flex-[0.95] flex-col items-center justify-center">
            ...
            <View className="mt-6 items-center justify-center">
              <View className="w-18 h-18 rounded-full bg-[#0891B2] items-center justify-center shadow">
                <Text className="text-white text-4xl">{safeTotalActive}</Text>
              </View>
            </View>
          </View>
        </View>
          ...
          >
            <View className="flex-row items-center justify-between">
              <Text className="text-sm text-gray-500">
                {safeCompletedSessions}/{safeTotalSessions} Active Sessions Done
              </Text>
            </View>
                ...
                <View key={pkg.id} className="flex-row items-center mb-2">
                  <View className="rounded-full bg-[#0891B2] px-2 py-0.5 mr-2">
                    <Text className="text-[#ffffff] text-xs font-semibold">
                      {pkg.currentSessions ?? 0}/{pkg.totalSessions ?? 0}
                    </Text>
                  </View>
