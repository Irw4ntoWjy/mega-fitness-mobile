...
const statusPill = (status: string) => {
  const base = "px-6 py-2 rounded-full border";
  switch (status) {
    case "Completed":
    ...
    <View style={{ flex: 1 }}>
      ...
      <ScrollView className="flex-1">
        <View className="px-6 mt-2">
          ...
          <Text className="text-3xl font-bold mt-5 tracking-tight">
            {tx.packageName}
          </Text>
          ...
          <View className="flex-row items-center justify-between mt-3">
            <View className="flex-row items-center gap-2">
              ...
              <Text className="text-zinc-800 font-semibold text-md">
                {tx.price}
              </Text>
            </View>
            ...
            <View className={statusPill(tx.status)}>
              <Text
                className={`text-md font-semibold ${statusText(tx.status)}`}
              >
                {tx.status}
              </Text>
            </View>
